...
namespace System.Text.Json.Serialization.Metadata
{
    ...
    /// </summary>
    internal sealed class TaggedPolymorphismResolver
    {
        ...
        public TaggedPolymorphismResolver(TaggedPolymorphicTypeConfiguration configuration)
        {
            Debug.Assert(!configuration.BaseType.IsValueType && !configuration.BaseType.IsSealed);

            _baseType = configuration.BaseType;

            foreach (KeyValuePair<Type, string> kvp in configuration.KnownTypes)
            {
        ...
        public static TaggedPolymorphismResolver? CreateFromAttributes(Type baseType)
        {
            if (!TaggedPolymorphicTypeConfiguration.TryCreateFromKnownTypeAttributes(baseType, out TaggedPolymorphicTypeConfiguration? config))
            {
                return null;
            }

            return new TaggedPolymorphismResolver(config);
        }
