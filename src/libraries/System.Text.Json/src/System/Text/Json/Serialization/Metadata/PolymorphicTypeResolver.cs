...
{
    /// <summary>
    /// JsonTypeInfo resolver for a given polymorphic base type.
    /// </summary>
...
#pragma warning disable CA2252 // This API requires opting into preview features
        ...
        public PolymorphicTypeResolver(JsonConverter baseConverter, IJsonPolymorphicTypeConfiguration configuration, JsonSerializerOptions options)
        {
            ...
            if (UsesTypeDiscriminators)
            {
                ...
                if (configuration.CustomTypeDiscriminatorPropertyName != null)
                {
                    CustomTypeDiscriminatorPropertyName = configuration.CustomTypeDiscriminatorPropertyName;
                    CustomTypeDiscriminatorPropertyNameUtf8 = Encoding.UTF8.GetBytes(CustomTypeDiscriminatorPropertyName);
            ...
            if (!_typeToDiscriminatorId.TryGetValue(runtimeType, out DerivedJsonTypeInfo? result))
            {
                switch (UnknownDerivedTypeHandling)
                {
                    ...
                    case JsonUnknownDerivedTypeHandling.FallbackToBaseType:
                        // Recover the polymorphic contract for the base type, if it exists.
