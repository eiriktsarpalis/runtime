...
namespace System.Text.Json.Schema.Tests
{
    public abstract partial class JsonSchemaExporterTests : SerializerTests
    {
        ...
        [JsonUnion(TypeClassifier = typeof(UnionOfDogOrCatClassifier))]
        public union UnionOfDogOrCat(UnionDog, UnionCat);
        ...
        public sealed class UnionOfDogOrCatClassifier : JsonTypeClassifierFactory<UnionOfDogOrCat>
        {
                ...
                JsonSerializerOptions options)
            {
                return static (ref Utf8JsonReader reader) =>
                {
                    ...
                    while (reader.Read() && reader.TokenType is not JsonTokenType.EndObject)
                    {
                        ...
                        {
                            if (reader.ValueTextEquals("Breed"u8)) return typeof(UnionDog);
                            if (reader.ValueTextEquals("Lives"u8)) return typeof(UnionCat);
                            reader.Read();
                            reader.TrySkip();
                        }
                    }
