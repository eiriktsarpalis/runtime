    ...
    public static partial class OptionsTests
    {
        ...
        private static JsonSerializerOptions GetFullyPopulatedOptionsInstance()
        {
            ...
            foreach (PropertyInfo property in typeof(JsonSerializerOptions).GetProperties())
            {
                ...
                else if (propertyType == typeof(IList<TaggedPolymorphicTypeConfiguration>))
                {
                    options.TaggedPolymorphicTypes.Add(new(typeof(ITestClass))
                    {
                        KnownTypes =
                        {
                            [typeof(Point_With_Array)] = "point_with_array",
                            [typeof(Point_With_Dictionary)] = "point_with_dictionary"
                        }
                    });
                }
                ...
                else if (propertyType == typeof(PolymorphicSerializationResolver))
                {
                    options.PolymorphicSerializationResolver = PolymorphicSerializationResolver.ObjectOnly;
                }
        ...
        private static void VerifyOptionsEqual(JsonSerializerOptions options, JsonSerializerOptions newOptions)
        {
            foreach (PropertyInfo property in typeof(JsonSerializerOptions).GetProperties())
            {
                ...
                {
                    var list1 = (IList<TaggedPolymorphicTypeConfiguration>)property.GetValue(options);
                    var list2 = (IList<TaggedPolymorphicTypeConfiguration>)property.GetValue(newOptions);
